...
不占手牌、也没有出牌时机：**每完成一圈自动抽一张，效果当场结算**——跟机会 / 命运共用同一套结算逻辑，
区别只在获取途径与偏向。整副 12 张整体偏正向：施工补贴 ¥800、贷款贴息 ¥600、「路网分红」按已完成的圈数
计价（每圈 ¥150，封顶 ¥1,500）、「免费加盖」添一栋房屋；也夹着违建拆除、年度审计、稽查传唤这类负项。

两条实现上的约束，改动时别踩：

- **免费加盖走 `canUpgradeLevel`，不是直接 `houses++`** —— 所以经典建房规则（集齐整组 / 均级建造 /
  同组无抵押）照样管得住它；当时没有可加盖的地产就折算成 ¥500 现金，不会抽到一张什么也不做的空牌。
- **结算挂在回合末尾，不塞进 `moveBy`** —— `moveBy` 会被卡牌的位移再次调用，挂在那里会让
  「机会卡前进 → 过起点 → 又抽技能卡 → 又位移」互相套娃。一回合内最多结算 3 张。

> **对局长度实测**（24 局无头回归）：开启 **272 回合**，关闭 **299 回合**；自然结束的比例从
> 50%（12/24）升到 62.5%（15/24）。变快的机制不是「补贴让人活得久」，而是免费加盖凭空多出房屋、
> 抬高了租金密度、加快了出清。不想要这套牌就在「设置 → 技能卡」里关掉，绕圈就不再发牌。
...
node tools/smoke.mjs --skills=0      # 关掉技能卡再跑一遍，对比对局长度
```
...
以及「同种子重跑结果一致」的确定性检查。技能卡另有 23 项：牌堆结构合法、绕圈发牌、
同一圈不重复发、关开关不发牌、牌堆抽空后重洗、分红封顶、免费加盖受经典规则约束、
无地可建时折算现金、存档保留牌堆。
...
图标是「环形棋盘（12 格 · 四组地产色）+ 中心骰子」的单层主体，刻意不做层层嵌套，
所以缩到 16px 剩下的仍是「蓝底 + 白环 + 金心」，而不是一团糊掉的小方块。改动时注意两点：
