...
| 建造/升级 | 再次停在**自己已买下**的地产/车站/水厂/电厂时，弹窗询问是否升级；最高满级。地产另有经典建房规则约束，见下方说明 |
...
只作用于**分组的地产**（沪西 / 法租界 / 市中心 / 北四川路 / 老城厢）：

- **集齐同组才能建房** —— 不然「集齐整组」这个目标没有分量，地价再高也只能收空地租金；
- **同组房屋数要均衡** —— 相差不超过 1 栋，不能把一块地直接堆到酒店；
- **同组不得有抵押** —— 抵押掉的那块会被对手忽略，等于拆自己的整组。

车站与公用事业没有分组、租金按持有数量计价，不受这三条限制。游戏内「规则」页有同样的说明。

> **为什么做成开关**：这三条是经典大富翁的设计，代价是对局明显变长。无头回归跑 12 局实测：
> 宽松规则平均 **150 回合**，经典规则平均 **310 回合**，且有一半对局打到 400 回合上限仍未结束
> —— 因为对手手里的关键地块只能靠掷骰子碰上，本作还没有玩家间交易。
> 想快节奏就在「设置 → 经典建房规则」里关掉，退回「单块地即可一路升到酒店」。
...
经过起点发薪、拍卖成交与流拍、卡牌位移、破产清算、经典建房的三种阻塞……），
以及「同种子重跑结果一致」的确定性检查。

随机对局按种子交替跑**经典 / 宽松两种建房规则**，收尾时会打印两者各自的平均回合数 ——
改完规则想确认对局长度有没有失控，看这一行就够。
